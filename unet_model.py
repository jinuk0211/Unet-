...
class unet(nn.Module):
  ...
  def forward(self,x):
    ...
    x = self.bottleneck(x)
    #순서 바꾸기 downsampling list 순서이므로 upsampling에 더해줄려면 역순이 되어야 한다
    skip_connections = skip_connections[::-1] #residual 에서 concat하기 위한 사전조치

    for idx in range(0,len(self.ups),2):
      x = self.ups[idx](x) #.conv2dtranspose만 down에서의 maxpool과 반대로 윗화살표 단계
      skip_connection = skip_connections[idx//2]
      # idx 가 0일떄 512번 output idx가 2일때 256 output~~
      # up은 transpose2d conv2d 둘다 append
      # down은 conv2d append 후
      # 생성자의 pool을 forward에서 따로 실행했으므로 idx//2

      if x.shape != skip_connection.shape:  #shape 변환을 통한 matrix addition
        x = tf.resize(x,size= skip_connection.shape[2:])

      concat_skip = torch.cat((skip_connection,x),dim=1) #skip_connection 더해준후
      x= self.ups[idx+1](concat_skip) #conv2d block 화살표 2개 진행
